...
function triangleCalc(form, editedField) {
    ...
    for (let field of triangleCalcFields) {
        if (parseFloat(form[field].value)) {
            if (!document.querySelector(`#${field} + label`).classList.contains("computed")) {
                document.querySelector(`#${field} + label`).classList.add("userProvided")
            } else {
                document.querySelector(`#${field} + label`).classList.remove("userProvided");
            }
        } else {
            document.querySelector(`#${field} + label`).classList.remove("userProvided");
        }
...
function heron(a, b, c) {
    ...
    // console.log(a.stringify(), b.stringify(), c.stringify());

    let s = fracMultiply(fracAdd(a, b, c), new frac(1, 2))
    let area = fracMultiply(s,
        fracAdd(s, fracMultiply(a, -1)),
        fracAdd(s, fracMultiply(b, -1)),
        fracAdd(s, fracMultiply(c, -1)),
    ) // actual area is this ^1/2
    console.log(s.stringify(), area.stringify());
    if (area.value() <= 0) {
        ...
        area = new frac(
            [
                [
                    [area.n[0][0][0], new frac(1, 2)]
                ]
            ],
            [
                [
                    [area.d[0][0][0], new frac(1, 2)]
                ]
            ]
        )
        console.log(area.stringify());
...
// checks if the number of finite elements exceed a certain threshold
